...
const EventTimeLine = ({ eventTimeline }) => {
    ...
    for (let i = 0; i < eventTimeline.length; i++) {
        if (eventTimeline[i].date.getTime() < new Date().getTime() && activeIndex === -1) {
            setActiveIndex(i);
            break;
        }
    };
                ...
                <VerticalTimeline layout="1-column-left" lineColor="#B57DFE" className="w-full" >
                    {
                        eventTimeline.map((data, index) => (
                            ...
                            >
                                <h3 className="text-lg font-bold sm:text-3xl">{data.title} | {data.date2 ? convertDate(data.date) + " - " + convertDate(data.date2) : convertDate(data.date)}</h3>
                                {
                                    data.bullets ? (
